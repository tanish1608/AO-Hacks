...
export function executeTool(tool: ToolId, state: Record<string, Json>, config: Record<string, Json>): Record<string, Json> {
 ...
 switch(tool){
  ...
  case 'deduplicate_records':{const seen=new Set<string>();const key=String(config.key??'id');s.records=rows(s.records).filter(r=>{if(r[key]===undefined||r[key]===null||r[key]==='')return true;const k=canonical(r[key]);if(seen.has(k))return false;seen.add(k);return true;});return s;}
  case 'filter_records':if(typeof config.field!=='string'||!Object.hasOwn(config,'value'))throw new Error('Filter needs field and value');s.records=rows(s.records).filter(r=>canonical(r[String(config.field)]??null)===canonical(config.value));return s;
  case 'sum_records':s.total=rows(s.records).reduce((sum,r)=>sum+numeric(r[String(config.field??'amount')]),0);s.count=rows(s.records).length;return s;
  case 'sort_jobs':{const key=String(config.by??'deadline');if(!['deadline','duration','value'].includes(key))throw new Error('Invalid job sort key');s.jobs=rows(s.jobs).sort((a,b)=>(numeric(a[key])-numeric(b[key]))*(config.direction==='desc'?-1:1));return s;}
